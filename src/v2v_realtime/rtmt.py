...
import aiohttp
from aiohttp import web, WSMessage, ClientWebSocketResponse
...
class RTMiddleTier:
    ...
    async def _process_message_to_client(
        ...
        server_ws: web.WebSocketResponse | ClientWebSocketResponse,
    ) -> Optional[str]:
                ...
                case "response.output_item.done":
                    if "item" in message and message["item"]["type"] == "function_call":
                        ...
                        try:
                            ...
                            logger.error(
                                f"Invalid JSON in function call arguments: {args} ({e})"
                            )
                            updated_message = None
                            return updated_message
                        maybe_coro = tool.target(parsed_args)
    ...
    async def _process_message_to_server(
        self, msg: WSMessage, ws: web.WebSocketResponse
    ) -> Optional[str]:
        message = json.loads(msg.data)
        updated_message = msg.data
        if message is not None:
        ...
        async with aiohttp.ClientSession(base_url=self.endpoint) as session:
            ...
            if self.key is not None:
                ...
                if self._token_provider is None:
                    raise RuntimeError("Token provider is not initialized.")
                headers = {
